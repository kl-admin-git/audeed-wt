...
$(document).ready(function () 
{
   ...
   {
       stringTarjetas += ComponenteTarjetaCredito('1','Roney Sebastian Rodriguez Huertas','4242 4242 **** 4242');       
   }

   $('#accordion').html(stringTarjetas);
});

function OnClickCambioTarjeta(control) 
{
    $('#accordion').find('.contenedorTarjeta ').removeClass('tarjetaSeleccionada');
    
    $(control).addClass('tarjetaSeleccionada');
}

function OnClickAgregarTarjeta() 
{
    ...
    if($('.departamentoPopUp').val() == 0){
        toastr.warning('Debes seleccionar el pais');
        return;
    }

    if($('.ciudadPopUp').val() == 0){
        toastr.warning('Debes seleccionar el pais');
        return;
    }
    ...
    // formData.append('nombreTarjeta',$('#nombreTarjeta').val());
    // formData.append('numeroTarjeta',$('#numeroTarjeta').val());
    // formData.append('fechaExpiracion',$('#fechaExpiracion').val());
    // formData.append('codigoSeguridad',$('#codigoDeSeguridad').val());

    // formData.append('direccion',$('#direccionUsuario').val());
    // formData.append('identificacion',$('#identificacion').val());
    // formData.append('telefono',$('#telefono').val());
    // formData.append('pais',$('.paisPopUp').find(':selected')[0].text);
    // formData.append('departamento',$('.departamentoPopUp').find(':selected')[0].text);
    // formData.append('ciudad',$('.ciudadPopUp').find(':selected')[0].text);
    ...
    let objetoEnviar =
    {
        ...
        pais:$('.paisPopUp').find(':selected')[0].text,
        departamento:$('.departamentoPopUp').find(':selected')[0].text,
        ciudad:$('.ciudadPopUp').find(':selected')[0].text,
...
function OnClickCancelarPopUpSuscripcion() 
{
    ...
    $('#identificacion').val('');
    $('#telefono').val('');
    $('.paisPopUp ').val(0).change();
    $('.departamentoPopUp ').val(0).change();
    $('.ciudadPopUp').val(0).change();
...
function OnClickSuscripcionPopUp() 
{
    $('#popUpCreditCard').modal('show');    
}

function ComponenteTarjetaCredito(
    idTarjetaCredito,
    nombreTarjeta,
    numeroTarjeta
) 
{
    ...
    string += ` <div class="card-header p-3 row m-b-10 contenedorTarjeta" onclick="OnClickCambioTarjeta(this);" idTarjetaCredito="${idTarjetaCredito}">

                    <div class="m-0 form-group col-lg-1">
                        <i class="ion-card iconCard"></i>
                    </div>
                    <div class="m-0 form-group col-lg-6">
                        <p>${nombreTarjeta}</p>
                        <p>${numeroTarjeta}</p>
                    </div>

                    <div class="m-0 form-group col-lg-5" style="text-align: end">
                        <i class="mdi mdi-delete trashIcon" idTarjetaCredito="${idTarjetaCredito}" onclick="event.stopPropagation(); OnClickEliminarTarjeta(this);"></i>
                    </div>
...
function OnClickEliminarTarjeta(control) 
{
    let idTarjetaCredito = $(control).attr('idTarjetaCredito');
    ...
    Swal.fire({
        ...
        $.ajax({
            type: 'POST',
            url: '/suscripciones/removeTarjetaCredito',
            data: 
            {
                _token: $('meta[name="csrf-token"]').attr('content'),
...
$('.agregarTarjeta').on('click',OnClickAgregarTarjeta);
$('.cancelarPopUpCreditCard').on('click',OnClickCancelarPopUpSuscripcion);
$('.addCard').on('click',OnClickSuscripcionPopUp);
