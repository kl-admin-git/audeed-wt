...
function IniciarVista() 
{
    ...
    $.ajax({
        type: 'POST',
        url: '/listachequeo/planaccion/manual/traerPlanesDeAccion',
        data: {
                    ...
                    { 
                        ...
                        if(item.tipo_plan_accion == 1){
                         ...
                         stringTabla += `<tr idAsignacion="${item.ID_EJECT_OPCIONES}">
                                            ...
                                            <td>${item.pregunta}</td>
                                            <td><span class="ellipseTextMultiLine" title="${(item.ES_RESPUESTA_ABIERTA == 1) ? item.RESPUESTA_ABIERTA : item.respuesta}">${(item.ES_RESPUESTA_ABIERTA == 1) ? item.RESPUESTA_ABIERTA : item.respuesta}</span></td>
...
{
    let url = window.location.origin + '/listachequeo/planaccion/manual';
