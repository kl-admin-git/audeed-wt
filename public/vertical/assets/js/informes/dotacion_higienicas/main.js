...
function IniciarVista() 
{
    ...
    $.ajax({
        type: 'POST',
        url: '/informes/dotacion/GetDataInit',
        data: {
        ...
        success: function(data)
        {
            ...
            switch (data.responseCode)
            {
                case 202:
                    ...
                    $('.tableDotacion tbody').html(stringTable);
...
async function OnClickViewRta(id_rta) 
{
    try 
    {
        ...
        let rs = await fetch(`/informes/dotacion/GetDataObsRta`, { method: "POST", body: data, headers:{
...
async function OnClickDownloadExcel()
{
    try 
    {
        ...
        CargandoNoMostrar();
        let rs = await fetch(`/informes/dotacion/DownloadExcel`, { method: "POST", body: data, headers:{
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        }});
        ...
        controlTemporal.download = "dotacion_practicas.xlsx";
    ...
    {
        CargandoNoMostrar();
        console.error(`Error al descargar excel de la dotación: ${error.message}`);
    }
...
{
    let url = window.location.origin + '/informes/dotacion_practicas';
