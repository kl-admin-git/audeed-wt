...
$('.input-number-decimal').on('input', function () { 
    this.value = this.value.replace(/[^0-9.-]/g,'');
});

$('.input-number-limit').on('input', function () 
{ 
    this.value = this.value.replace(/[^0-9.]/g,'');
...
function ValidarExtensionImagen(imagenNombre) 
{
    var extension = imagenNombre.split('.').pop().toUpperCase();
...
function ValidarSiTieneDatos(contenedor) 
{
    if($(contenedor).find('div').length == 0)
        $('#main_no_data').removeClass('hidden');
    else
        $('#main_no_data').addClass('hidden');
}
...
$(document).ready(function() 
{
    if(estaSuscrito == 0)
    {
        ...
        $('body').attr('onkeydown','return false');
        $('#popUpSuscripcion').modal('show');
    }

});

function OnClickSuscribirme() 
{
    let idSuscripcion = $(this).attr('idSuscripcionQuemada');

    $.ajax({
        type: 'POST',
        url: '/suscripciones/seleccionarPlan',
        data: 
        {
            ...
            switch (data.codigoRespuesta) 
            {
                case 206:
                    ...
                    Swal.fire(
                        ...
                        $('body').attr('onkeydown','');
                        $('#popUpSuscripcion').modal('hide');            
                    });;
                    
                    break;

                case 204: // NO TIENES TARJETA DE CRÉDITO ASIGNADA
                        $('body').attr('onkeydown','');
                        $('#popUpCreditCard').modal('show');
                    break;
...
function OnClickSuscribirmePagoUnico() 
{
    let idSuscripcion = $(this).attr('idSuscripcionQuemada');
    let amount = parseFloat($(this).parents().eq(1).find('.valorPlan').attr('valor')).toFixed();
    switch (idSuscripcion) {
        ...
        case '2': // 85000
            $.ajax({
                type: 'POST',
                url: '/suscripciones/validarSuscripcion',
                data: 
                {
                    _token: $('meta[name="csrf-token"]').attr('content')
                },
                ...
                success: function(data) 
                {
                    ...
                    {
                        case 202:
                            $('.description').val('Pago suscripción -'+idSuscripcion);
                            $('.amount').val(amount);
                            $('.currency').val(currency);
                            $('.buyerEmail').val(emailCuentaPrincipal);
                            var signature = hex_md5(ApiKey+"~"+merchantId+"~"+referenceCode+"~"+amount+"~"+currency);
                            $('.signature').val(signature);
                            $('#formularioPayuSuscripcion').submit();
                            break;
        ...
        case '4':// CONTACTANOS
            ...
            $.ajax({
                type: 'POST',
                url: '/suscripciones/enviarCorreoContactanos',
                data: 
                {
                    _token: $('meta[name="csrf-token"]').attr('content')
                },
...
function OnClickCancelarPopUpSuscripcion() 
{
    $('#nombreTarjeta').val('');
    $('#numeroTarjeta').val('');
    $('#fechaExpiracion').val('');
    $('#codigoDeSeguridad').val('');
    $('#direccionUsuario').val('');
    $('#identificacion').val('');
    $('#telefono').val('');
    $('.paisPopUpMainPage ').val(0).change();
    $('.departamentoPopUpMainPage ').val(0).change();
    $('.ciudadPopUpMainPage').val(0).change();
    
    $('.creditcard').removeClass('flipped');
    $('#svgnumber').html('*** *** **** ****');
    $('#svgname').html('TU NOMBRE');
    $('#svgexpire').html('01/23');
    $('#svgsecurity').html('000');
    $('#svgnameback').html('TU NOMBRE');
    $('#popUpCreditCard').modal('hide');
}

function OnChangePaisMainPage() 
{
    let idPais = $(this).val();
    $.ajax({
        type: 'POST',
        ...
        success: function(data) 
        {
            ...
            switch (data.codigoRespuesta) 
            {
                case 202:
                    // CARGA DEPARTAMENTOS
                    $('.departamentoPopUpMainPage').html('');

                    $('.departamentoPopUpMainPage')
                    ...
                    $(".departamentoPopUpMainPage").select2({}); 

                     // CARGA CIUDADES
                     $('.ciudadPopUpMainPage').html('');
...
function OnChangeDepartamentoMainPage() 
{
...
function OnClickAgregarTarjetaMainPage() 
{
    if($('#nombreTarjeta').val() == ''){
        toastr.warning('Debes escribir el nombre de la tarjeta');
        $('#nombreTarjeta').focus();
        return;
    }

    if($('#numeroTarjeta').val() == ''){
        toastr.warning('Debes escribir el número de la tarjeta ');
        $('#numeroTarjeta').focus();
        return;
    }
    
    if($('#fechaExpiracion').val() == ''){
        toastr.warning('Debes la fecha de expiración de la tarjeta');
        $('#fechaExpiracion').focus();
        return;
    }
    if($('#codigoDeSeguridad').val() == ''){
        toastr.warning('Debes escribir el código de seguridad (CVV)');
        $('#codigoDeSeguridad').focus();
        return;
    }

    if($('#numeroTarjeta').val().length != 19){
        toastr.warning('Debes escribir completo el número de la tarjeta');
        $('#numeroTarjeta').focus();
        return;
    }

    if($('#direccionUsuarioMainPage').val() == ''){
        toastr.warning('Debes escribir la dirección');
        $('#direccionUsuarioMainPage').focus();
        return;
    }

    if($('#identificacionMainPage').val() == ''){
        toastr.warning('Debes escribir el número de identificación');
        $('#identificacionMainPage').focus();
        return;
    }

    if($('#telefonoMainPage').val() == ''){
        toastr.warning('Debes escribir el número de teléfono');
        $('#telefonoMainPage').focus();
        return;
    }

    if($('.paisPopUpMainPage').val() == 0){
        toastr.warning('Debes seleccionar el pais');
        return;
    }

    if($('.departamentoPopUpMainPage').val() == 0){
        toastr.warning('Debes seleccionar el pais');
        return;
    }

    if($('.ciudadPopUpMainPage').val() == 0){
        toastr.warning('Debes seleccionar el pais');
        return;
    }

    if(tipoTarjeta == 'Unknown')
    {
        toastr.warning('Tarjeta invalida');
        return;
    }
    ...
    let objetoEnviar =
    {
        ...
        codigoSeguridad:$('#codigoDeSeguridad').val(),
        direccion:$('#direccionUsuarioMainPage').val(),
        identificacion: $('#identificacionMainPage').val(),
        telefono:$('#telefonoMainPage').val(),
        pais:$('.paisPopUpMainPage').find(':selected')[0].text,
        departamento:$('.departamentoPopUpMainPage').find(':selected')[0].text,
        ciudad:$('.ciudadPopUpMainPage').find(':selected')[0].text,
    ...
    $.ajax({
        ...
        type: 'POST',
        url: '/suscripciones/agregarTarjeta',
        data: formData,
        cache: false,
        processData: false, 
        contentType: false, 
            ...
            switch (data.codigoRespuesta) 
            {
                case 206:
                    Swal.fire(
                        {
                            title: data.mensaje,
                            text: '¡Ahora si, selecciona tu suscripción!',
                            type: 'success',
                        ...
                        OnClickCancelarPopUpSuscripcion();
                    });
                    break;
...
$('.suscribirme').on('click',OnClickSuscribirme);
$('.cancelarPopUpCreditCard').on('click',OnClickCancelarPopUpSuscripcion);
$('.paisPopUpMainPage').on('change',OnChangePaisMainPage);
$('.departamentoPopUpMainPage').on('change',OnChangeDepartamentoMainPage);
$('.agregarTarjetaMainPage').on('click',OnClickAgregarTarjetaMainPage);

$('.suscribirmePagoUnico').on('click',OnClickSuscribirmePagoUnico);
// LOGICA PARA TARJETA DE CREDITO INPUTS
window.onload = function () {

    const name = document.getElementById('nombreTarjeta');
    const cardnumber = document.getElementById('numeroTarjeta');
    const expirationdate = document.getElementById('fechaExpiracion');
    const securitycode = document.getElementById('codigoDeSeguridad');
    ...
    cardnumber_mask.on("accept", function () {
        ...
        tipoTarjeta = cardnumber_mask.masked.currentMask.cardtype;
